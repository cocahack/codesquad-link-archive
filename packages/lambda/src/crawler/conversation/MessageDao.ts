...
export default class MessageDao {
  ...
  async findAll(channelId: string): Promise<Slack.Conversation.SlackMessage[]> {
    const args: ConversationsHistoryArguments = {
      channel: channelId,
      oldest: moment().subtract(1, 'days').startOf('day').unix().toString(),
      latest: moment().subtract(1, 'days').endOf('day').unix().toString(),
    };

    const res: ConversationHistoryResponse = await this.webClient.conversations.history(args);
